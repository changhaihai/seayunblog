...
image: /pic/cf-fastip-optimization/cf-fastip-11.avif
...
![未优选效果](/pic/cf-fastip-optimization/098f9ee71ae62603022e542878673e19bdcaf196.avif)

#### 已优选

![已优选效果](/pic/cf-fastip-optimization/cf-fastip-11.avif)
...
![ITDog测试](/pic/cf-fastip-optimization/838f685e-3913-4b21-995e-5ee149f4bffa.avif)
...
![Worker路由配置](/pic/cf-fastip-optimization/cf-fastip.avif)

3. 写一条DNS解析到想要的优选域名

![DNS解析](/pic/cf-fastip-optimization/cf-fastip-1.avif)
...
![创建路由](/pic/cf-fastip-optimization/56752d54-26a5-46f1-a7d9-a782ad9874cb.avif)

![路由配置](/pic/cf-fastip-optimization/d025398c-39e3-4bd7-8d8f-2ce06a45007d.avif)
...
![创建R2实例](/pic/cf-fastip-optimization/cf-fastip-4.avif)

2. 绑定一个自定义域

![绑定自定义域](/pic/cf-fastip-optimization/cf-fastip-5.avif)

3. 前往域名 → 规则 → Cloud Connector

![Cloud Connector](/pic/cf-fastip-optimization/cf-fastip-6.avif)

![配置Cloud Connector](/pic/cf-fastip-optimization/cf-fastip-7.avif)

![Cloud Connector配置](/pic/cf-fastip-optimization/cf-fastip-8.avif)
...
![SaaS配置示例1](/pic/cf-fastip-optimization/cf-saas-1.avif)

![SaaS配置示例2](/pic/cf-fastip-optimization/cf-saas-2.avif)

![SaaS配置示例3](/pic/cf-fastip-optimization/cf-saas-3.avif)

Worker中的路由规则也适用：

![Worker路由规则](/pic/cf-fastip-optimization/cf-saas-4.avif)
...
![DNS解析配置](/pic/cf-fastip-optimization/c94c34ee262fb51fb5697226ae0df2d804bf76fe.avif)

2. 前往辅助域名的SSL/TLS → 自定义主机名，设置回退源为第一步的DNS解析域名（推荐HTTP验证）

3. 添加自定义主机名，选择自定义源服务器，填写第一步的域名

![自定义主机名配置](/pic/cf-fastip-optimization/f6170f009c43f7c6bee4c2d29e2db7498fa1d0dc.avif)

4. 在辅助域名添加一条解析，CNAME到优选节点，不开启CF代理

![优选节点解析](/pic/cf-fastip-optimization/4f9f727b0490e0b33d360a2363c1026003060b29.avif)

5. 在主力域名添加解析，域名为自定义主机名，目标为刚才的cdn域名，不开启CF代理

![主力域名解析](/pic/cf-fastip-optimization/6f51cb2a42140a9bf364f88a5715291be616a254.avif)

6. 优选完毕，确保优选有效后尝试访问

![优选完成](/pic/cf-fastip-optimization/cf-fastip-10.avif)
...
![Worker路由](/pic/cf-fastip-optimization/cf-fastip-12.avif)

![外域路由](/pic/cf-fastip-optimization/cf-fastip-13.avif)

### Cloudflare Tunnel（ZeroTrust）

先参照传统SaaS优选设置完毕，源站即为Cloudflare Tunnel。

![Tunnel配置](/pic/cf-fastip-optimization/cf-fastip-2.avif)

![Tunnel规则](/pic/cf-fastip-optimization/cf-fastip-3.avif)
